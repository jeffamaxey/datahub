...
df1 = spark.read.option("header", "true").csv(f"{DATA_DIR}/in1.csv")
df2 = spark.read.option("header", "true").csv(f"{DATA_DIR}/in2.csv")
...
df.write.mode('overwrite').csv(f"{DATA_DIR}/{TEST_CASE_NAME}/out1.csv")

df_o = spark.sql("select v1.c1 as a1, v1.c2 as b1, v2.c1 as c1, v2.c2 as d1 from v1 join v2 on v1.id = v2.id")
#InsertIntoHadoopFsRelationCommand
df_o.write.mode('overwrite').csv(f"{DATA_DIR}/{TEST_CASE_NAME}/out2.csv")
