...
def build_hive_dataset_mce(dataset_name, schema, metadata):
    ...
    """
    actor, type, created_time, upstreams_dataset, sys_time = (
        f"urn:li:corpuser:{metadata[2][7:]}",
        str(metadata[-1][11:-1]),
        int(metadata[3][12:]),
        metadata[-28][10:],
        int(time.time()),
    ...
    upstreams = {
        "upstreams": [
            {
                "auditStamp": {"time": sys_time, "actor": actor},
                "dataset": f"urn:li:dataset:(urn:li:dataPlatform:hive,{upstreams_dataset},PROD)",
                "type": "TRANSFORMED",
            }
        ]
    }
    ...
    mce = {
        ...
        "proposedSnapshot": {
            "com.linkedin.pegasus2avro.metadata.snapshot.DatasetSnapshot": {
                "urn": f"urn:li:dataset:(urn:li:dataPlatform:hive,{dataset_name},PROD)",
                "aspects": [
...
for database in databases:
    tables = hive_query(f'show tables in {database[0]}')
    for table in tables:
        dataset_name = f'{database[0]}.{table[0]}'
        description = hive_query(f'describe extended {dataset_name}')
